...
function save_content(tt){
	...
	if($.trim(content)=="" || $.trim(content)=="最多输入300字，班主任评语与学籍管理云平台的学生操行评语实时互通，只需在一个平台录入，两个平台皆可显示。" || $.trim(content)=="最多输入4000字"){
		isSave=0;
		tt.parent().css("color","");
		if(sectionCode=='2030'){
			input_noticemaster(newId,300);
		}else{
...
function update_content(tt,num){
	...
	if($.trim(val_now)=="" || $.trim(val_now)=="最多输入4000字" || $.trim(val_now)=="最多输入300字,班主任评语与学籍管理云平台的学生操行评语实时互通，只需在一个平台录入，两个平台皆可显示。"){
		isNull=1;
		return;
	}
	if(sectionCode=='2030' && val_now.length>300){
		isNull=1;
		apprasial_alert(300);
		return;
	}
...
function addContent(parentDiv,currentObj){
	...
	var tmp= '<table  class="xiechengsheng4" cellspacing="0" id="'+sid+'_tab">' 
        ...
        + '<td  class="xiechengsheng" >评价人：班主任 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签名：'+teacherName+'&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
...
function deleteContent(currentObj,stuDivObj){
	...
	Ext.Ajax.request({
		url:'${ctx}/master/MasterApriseDataAction.a?doDeleteMasterAppraisal',
		method:'POST',
		defaults:{autoScroll: true},
		success:function(response,options){
			var temp=response.responseText;
		    if(temp!="false"){
		    	...
		    	if(count==1){
					...
					if(sectionCode=="2030"){
						input_noticemaster(studentid+"_newAdd",300)
					}else{
		    	...
		    	if(count==1){
		    		...
		    		if(textArea.val()=="最多输入4000字" || textArea.val()=="最多输入300字，班主任评语与学籍管理云平台的学生操行评语实时互通，只需在一个平台录入，两个平台皆可显示。"){
...
function chooseClass(){
	...
	ShowDiv();
	window.location.href="${ctx}/master/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+evaluatetypeid+"&&sectionName="+sectionName+"&&classId="+classid;
}
//锚点选中数据
...
function chooseTerm(){
	...
	ShowDiv();
	window.location.href="${ctx}/master/MasterApriseDataAction.a?showAppriseList&&flag="+'${flag}'+"&&evaluatetypeid="+'${evaluatetypeid}'+"&&sectionName="+'${sectionName}'+"&&classId="+'${classId}'+"&&zsTermId="+zsTermId+"&&isHistory=1";
}
...
<div class="dangqianwz">
    ...
    <span >班级：
     <select id="selectedId" name="select" class="  wenziliebiao100" onchange="chooseClass();">
     			<!-- <option value="111">wewewe</option> -->
     			<c:if test="${not empty classInfos }">  
     				<c:forEach items="${classInfos}" var="cInfo"  varStatus="vs">
     					<%-- <option "${vs.count==1?'selected=selected':'' }" value="${fn:split(cInfo,'@')[1] }">${fn:split(cInfo,'@')[0] }</option> --%>
...
<c:forEach items="${ appriseInfos}" var="student" varStatus="vst">
	<div id="pj_dengsl_main" name="${student.eduid}_pic" style="background:white; padding-top:18px;width:95%;margin-left:3%;<c:if test='${vst.index!=0 }'>display:none;</c:if>" class="dslxingmingdiv" >
		<div class="name_pic">
			<%-- <img class="img-padding" src="${ctx}/imgs/pic1.png" /> --%>
    	...
    	<c:forEach items="${student.aInfos}" var="info" varStatus="vs">
   			<table  class="xiechengsheng4" cellspacing="0"  id="${info.apprasialid}_tab"> 
		           <tr> 
		                ...
		                <td  class="xiechengsheng" >评价人：班主任 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签名：<span id="${info.apprasialid}_sign">${info.appraser }</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		        ...
		        <div class="name_con" id ="${info.apprasialid}_one" style="background:#eee">
		          ...
		          <div class="name_cxbox" id ="${info.apprasialid}_two"
		          		 <c:if test="${vs.count != fn:length(student.aInfos) or evaluatetypeid eq '2030'}">
					                  	  style="display:none;"
				                  </c:if>
		          >

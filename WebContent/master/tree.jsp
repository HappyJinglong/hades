...
<script type="text/javascript">
var dataUrl = '${ctx}/master/TreeNodeAction.a?getTreeContent';	
var menutree;
...
function forward(){
	...
	if(commonFuncId=='null'||commonFuncId==''||commonFuncId=='3010'){
		id="student";
		$("#"+id).attr("href","${ctx }/master/StudentApprisedAction.a?showAppriseCount");
	}else{
...
var timeId = setInterval("clearTimeId()",5);
  function showStudentMenu(){
	  self.parent.frames[2].ShowDiv && self.parent.frames[2].ShowDiv();
	    ...
	    $("#student").attr("href","${ctx }/master/StudentApprisedAction.a?showAppriseCount");
  }
  function showTeacherMenu(){
	  self.parent.frames[2].ShowDiv && self.parent.frames[2].ShowDiv();
	    ...
	    $("#teacher").attr("href","${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=2030");
  }
...
<div id="pj_menu2"  style="display:none;width: 100%;"> 
    <ul class="menu menu_zw" style="overflow:auto;width: 100%;overflow-x:hidden; height: 100%;">
     ...
     <%--  <li class="cur"><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=2030" target="main" class="nobdt">班主任评语&nbsp;<span class="red">*</span></a></li>  --%>
      <li class="cur"><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=2030" <c:if test="${ empty commonFuncId or commonFuncId eq '3010'}">id="student"</c:if> <c:if test="${not empty commonFuncId and commonFuncId ne '3010'}">id="student9998"</c:if> target="main" onclick="changeAStyle($(this))"><img src="${ctx}/images/xiesh.png">班主任评语&nbsp;<span class="red">*</span></a></li>
      <li><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=3020" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1003"</c:if> target="main" class="nobdt" onclick="changeAStyle($(this))"><img src="${ctx}/images/xiesx.png">思想道德</a></li> 
      <li><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=8040" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1004"</c:if> target="main"  onclick="changeAStyle($(this))"><img src="${ctx}/images/xiecj.png">学业成就&nbsp;</a></li> 
      <li><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=4020" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1005"</c:if> target="main"  onclick="changeAStyle($(this))"><img src="${ctx}/images/xiehz.png">合作与交流</a></li>  
      <li><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=5020" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1006"</c:if> target="main"  onclick="changeAStyle($(this))"><img src="${ctx}/images/xieyd.png">运动与健康</a></li>  
      <li><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=6020" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1007"</c:if> target="main"  onclick="changeAStyle($(this))"><img src="${ctx}/images/xiesm.png">审美与表现</a></li>  
      <li><a href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode=7030" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1009"</c:if> target="main"  onclick="changeAStyle($(this))"><img src="${ctx}/images/xiegx.png">个性发展</a></li> 

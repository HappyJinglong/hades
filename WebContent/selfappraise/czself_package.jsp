...
.wenbenkuang670{ width:68%; height:26px; line-height:26px; border:1px solid #999;}
</style>
...
<input type="hidden" name="evaluateType1" id="evaluateType1" value="31"> 
<input type="hidden" name="evaluateType2" id="evaluateType2" value="35"> 
</c:if>
<c:if test="${choicenum eq PAGE_HZJL}">
<input type="hidden" name="evaluateType1" id="evaluateType1" value="51"> 
<input type="hidden" name="evaluateType2" id="evaluateType2" value="55">
</c:if>
<c:if test="${choicenum eq PAGE_SMYBX}">
<input type="hidden" name="evaluateType1" id="evaluateType1" value="71"> 
<input type="hidden" name="evaluateType2" id="evaluateType2" value="75"> 
</c:if>
<c:if test="${choicenum eq PAGE_GXFZ}">
<input type="hidden" name="evaluateType1" id="evaluateType1" value="91"> 
<input type="hidden" name="evaluateType2" id="evaluateType2" value="95"> 
</c:if>
<input type="hidden" name="choicenum" id="choicenum" value="${choicenum}"> 
	...
	<c:if test="${choicenum eq PAGE_GXFZ}"><span class="fl">当前位置：自我评价->个性与发展</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</c:if>
	<span>学期：
			...
			<div id="new_down" >
				<table width="100%" id="new_table2" border="0" cellspacing="1" bgcolor="#999999"
					...
					<tr class="title_bg">
						...
						<c:if test="${choicenum eq PAGE_GXFZ}"><td colspan="3">特长与成果展示&nbsp;<span class="red">*</span></td></c:if>
					</tr>
					<tr id="new_tr2">
					...
					<tr>
						<td colspan="2" class="h50" id ="new_td2" >
							<div class="downbox mt18">
								<span class="fl wenben85left">主题：&nbsp;&nbsp;&nbsp;&nbsp;</span> 
								<input type="text" class="fl wenbenkuang670" id="new_topic2"  maxlength="25" style="background: transparent;float:left;color:#BCBCBC;cursor:text;" onfocus="turnon1('new_topic2',this)" onblur="turnoff1('new_topic2',this)" value="最多输入25字" />
							</div>
							<div class="downbox mt18">
						...
						<td width="100" height="191" class="bg_eee" style="margin:0px auto;float:none">
			...
			<div id="new_down" >
				<table width="100%"  id="new_table2" border="0" cellspacing="1" bgcolor="#999999"
					...
					<tr class="title_bg">
						...
						<c:if test="${choicenum eq PAGE_HZJL}"><td colspan="3">合作与交流行为记录袋</td></c:if>
						<c:if test="${choicenum eq PAGE_SMYBX}"><td colspan="3">审美与表现记录袋&nbsp;<span class="red">*</span></td></c:if>
						<c:if test="${choicenum eq PAGE_GXFZ}"><td colspan="3">特长与成果展示</td></c:if>
					</tr>
					<%int i=1;%>
					<c:forEach items="${appraisalList2}" var="list" varStatus="status">
						...
						<tr>
							<td colspan="2" id="${list.part_id}_td" style="background:#eee;">
							<div class="downbox mt18">
								<span class="fl wenben85left">主题：&nbsp;&nbsp;&nbsp;&nbsp;</span> 
								<input type="text" style="background: transparent;" class="fl wenbenkuang670"  maxlength="25" id="${list.part_id}_topic" value="${list.topic}" onfocus="turnon1('${list.part_id}_topic',this)" onblur="turnoff1('${list.part_id}_topic',this)"  onclick="changeBackgroundtd('${list.part_id}')"/>
							</div>
								<div class="downbox mt18">
								<span class="fl wenben85left">内容描述：</span>
				 					 <textarea class="content"
						                ...
						                onclick="changeBackgroundtd('${list.part_id}');"
						                 onfocus="turnon('new_content2',this)" onblur="turnoff('new_content2',this)"
								...
								<div id="${list.part_id}_attach">
									...
									<c:forEach items="${list.attachListForFile}" var="attachFile">
		            					...
		            					&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" title="删除" onclick="deleteselfattach('${attachFile.attachment_id}','${list.part_id}')"><img src="${ctx}/images/upload_del.gif" style="height:10px; width:10px; " border="0"/></img></a></span> 
			            						<%-- ${attachFile.attachment_name}
							...
							<input type="button" class="fr shanchu1" title="删除" style="margin:0px auto;float:none" id="${list.part_id}_delete2" onclick="deleteselfpackage(this)"/> 
							<input type="button" class="fr tianjia" title="保存" style="margin:0px auto;float:none" id="${list.part_id}_save"  onclick="updateold('${list.part_id}')"/>
							</td>
						</tr>
						<%i++;%>
					</c:forEach>
		 ...
		 function savepackage(div,day,valuenum,rid,zhuti,check){
			if(saveNow == 0){
				...
				if((document.getElementById(zhuti).value.replace(/(^\s*)|(\s*$)/g,'')!= "")&&(document.getElementById(zhuti).value!= "最多输入25字")){
					if(document.getElementById(zhuti).value.length<=25){
						if(document.getElementById(valuenum).value.length<=600){
							...
							saveNow = 1;
							Ext.Ajax.request({
								url:'${ctx}/partinfo/PartinfoAction.a?doInsertSelfAppWithatt',
								method:'POST',
			 		...
			 		if(Sys.ie<7.0){
						...
						return;
			 			 ...
			 			 var cc = document.getElementById("choicenum").value;
							...
							if(document.getElementById(id+"_topic").value.replace(/(^\s*)|(\s*$)/g,'').length!=0&&document.getElementById(id+"_topic").value!="最多输入25字"){
											...
											if(tr.size()==4){
												...
												var str="";
												 str+="<tr id='new_tr2'><td width='10%' height='40' id='new_num2' class='bg_eee'>1</td><td colspan='2' class='youjuzhong pr20 bg_eee'>"
													...
													+"</td></tr><tr><td colspan='2' id='new_td2'  class='h50'><div class='downbox mt18'>"
												 	+"<span class='fl wenben85left'>主题：&nbsp;&nbsp;&nbsp;&nbsp;</span><input type='text' maxlength='25' class='fl wenbenkuang670' id='new_topic2'  style='background: transparent; float:left;color:#BCBCBC;cursor:text;' onfocus='turnon1(\""+"new_topic2"+"\",this)' onblur='turnoff1(\""+"new_topic2"+"\",this)' value='最多输入25字' /></div>"
		...
		/* 	event = event? event: window.event;
			...
			if(idi!="new"){
				...
				var type2 = document.getElementById("evaluateType2").value;
					if(document.getElementById(idi+"_topic").value.replace(/(^\s*)|(\s*$)/g,'') != ""||document.getElementById(idi+"_topic").value.replace(/(^\s*)|(\s*$)/g,'') != "最多输入25字"){
						if(document.getElementById(idi+"_topic").value.length<=25){
						if(document.getElementById(idi+"_content").value.length<=4000){
							...
							var v_url='${ctx}/partinfo/PartinfoAction.a?doUpdataSelfPartInfo'
							Ext.Ajax.request({
								url : v_url,
								method:'POST',
								defaults:{autoScroll: true},

...
<un:bind var="PAGE_XYCJ" type="com.flyrish.hades.common.Constant" field="PAGE_XYCJ" />
<un:bind var="TYPE_XY_ZWPJ" type="com.flyrish.hades.common.Constant" field="TYPE_XY_ZWPJ" />
...
<input type="hidden" name="evaluateType1" id="evaluateType1" value="${TYPE_XY_ZWPJ}"> 
<input type="hidden" name="evaluateType2" id="evaluateType2" value="1"> 
...
<input type="hidden" name="evaluatePersonName" id="evaluatePersonName" value="${evaluatePersonName}"> 
	...
	<span class="fl">当前位置：自我评价->学业成就</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
  	<span>学期：
	...
	<div id="top" class="top">
		...
		<div id="down" class="down mt18 pb30">
			<c:if test="${empty learnprocessWorksList}">
			<div id="new_down" >
				<table width="100%" id="new_table2" border="0" cellspacing="1" bgcolor="#999999"
					class="biaoge">
					<tr class="title_bg">
						<td colspan="3">学科学习过程记录<span class="red"></span></td>
					</tr>
					<tr id="new_tr2">
					...
					<tr>
						<td colspan="2" class="h50" id ="new_td2" style="width: 90%;">
                    		...
                    		<span class="fl wenben85left">科目：</span>
                    			<div align="left">
                    			<select name="new_subjectName" id="new_subjectName" style="background: transparent;border:1px solid #999;">
		               				<app:subjectSelectTag levelCode="${levelcode}"/>
		              			</select>
		              			</div>
               				 </div>
							<div class="downbox mt18">
								<span class="fl wenben85left">内容描述：</span>
								<textarea id="new_content2" style="background: transparent; overflow:auto; height: 130px; width: 99%; border:1px solid #999; float:left;color:#BCBCBC;cursor:text;" onfocus="turnon('new_content2',this)" onblur="turnoff('new_content2',this)">最多输入200字</textarea>
							</div>
							<div id="new_attach">
					...
					<tr id="addpackagelocation">
						<td colspan="3" class="bg_eee h80">
							<div class="fr btn">
								<input type="button" id = "add1" title="增加" style="margin-right :18px;" onclick="addnew1()"  style="margin-right :10px;" class="fr shanchu" />
							</div>
						</td>
					</tr>
				</table>
			...
			<c:if test="${not empty learnprocessWorksList}">
			<div id="new_down" >
				<table width="100%"  id="new_table2" border="0" cellspacing="1" bgcolor="#999999"
					class="biaoge">
					<tr class="title_bg">
					...
					<%int i=1;%>
					<c:forEach items="${learnprocessWorksList}" var="list" varStatus="status">
						<tr id="${list.workid}_tr">
							<td width="10%" height="40 " class="bg_eee" id="${list.workid}_num"><%=i%></td>
							   <td colspan="2" class="youjuzhong pr20 bg_eee" >
							   		评价人：本人 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        			签名：${evaluatePersonName}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        			日期：<input id="${list.workid}_startDate" type="text" value="${list.signdate}" size="10" onClick="WdatePicker()"/> 
							   </td>
						</tr>
						<tr>
							<td colspan="2" id="${list.workid}_td" style="background:#eee;width: 90%;">
								<input type="hidden" style="background: transparent;" class="fl wenbenkuang670" id="${list.workid}_topic" onclick="changeBackgroundtd('${list.workid}')"/>
							<div class="downbox mt18">
                    		<span class="fl wenben85left">科目：</span>
                    		<div align="left">
                    			<select name="${list.workid}_subjectName" id="${list.workid}_subjectName" style="background: transparent;border:1px solid #999;">
		               				<app:subjectSelectTag levelCode="${levelcode}" subjectName="${list.subject}"/>
		              			</select>
		              		</div>
               				 </div>
								<div class="downbox mt18">
								<span class="fl wenben85left">内容描述：</span>
				 					 <textarea class="content"
						                id="${list.workid}_content"
						                style="background: transparent; overflow:auto; height: 130px; width: 99%; border:1px solid #999;"
						                idvalue="${list.processdesc}"
						                onclick="changeBackgroundtd('${list.workid}');"
						                onfocus="turnon('${list.workid}',this)" onblur="turnoff('${list.workid}',this)"
						                >${list.processdesc}</textarea>
								</div>
								<div id="${list.workid}_attach">
										<%int j=1;%>
									<c:forEach items="${list.attachListForFile}" var="attachFile">
										<div class="downbox mt18" id="${attachFile.attachid}_attachment">
		            					<span class="fl wenben85left">附件<%=j %>：&nbsp;&nbsp;&nbsp;
		            						<a href="javascript:void(0);" style="color:black;" onclick="dodown('${attachFile.attachid}','${list.workid}')">${attachFile.filename}</a>
		            						&nbsp;&nbsp;&nbsp;&nbsp;<a href="javascript:void(0);" title="删除"  onclick="deleteselfattach('${attachFile.attachid}','${list.workid}')"><img src="${ctx}/images/upload_del.gif" style="height:10px; width:10px; "border="0"/></img></a>
		            						</span>
			            						<%-- ${attachFile.filename}
			            						<input type="button" value="下 载" class="shangchuan fl ml10"  onclick="dodown('${attachFile.attachid}')"/> 
												<input type="button" value="删 除" class="shangchuan fl ml10" onclick="deleteselfattach('${attachFile.attachid}','${list.workid}')"/> --%>
	            						<%j++;%>
	            						</div>
            						</c:forEach>
            					</div>	
								<div class="downbox mt18" style="position: relative;">
									<form method="post" id="${list.workid}_frm">
									<input type="hidden" name="${list.workid}_old_uuid" id="${list.workid}_old_uuid" value="${uuid}"/>
									<input type="hidden" name="${list.workid}_rpID" id="${list.workid}_rpID" value="${list.workid}"/>
									<span class="fl wenben85left">附件：</span> 
<%-- 									<input name='${list.workid}_textfield' id='${list.workid}_textfield' type="text"
										class="wenbenkuang670 fujian"  style="background: transparent;position:absolute;left:80px;"
										value="格式仅限doc、docx、txt、jpg、pdf、pps、ppsx、ppt和pptx" readonly/>
									<span class="doupload">
									<input type="file" class="fileadd2" size="28" name="${list.workid}_fileField" id="${list.workid}_fileField" style="margin-left:130px"
									onchange="document.getElementById('${list.workid}_textfield').value=this.value"/><span id="${list.workid}_tp" style="margin-left:100px">0%</span>  
									<input type="button" value="添 加" class="shangchuan ml10" style="margin-left:20px" /> 
									<input type="button" value="上 传" id ="${list.workid}_upload" onclick="return beforeSubmitForm('old_fileField','${list.workid}_frm','${list.workid}_old_uuid','${list.workid}_tp','${list.workid}');"
										class="shangchuan ml10" /> </span>--%> 
									<input name='${list.workid}_fileField' id='${list.workid}_fileField' type="file"
										  style="position:absolute;background: transparent;left:80px;" />
									<input id="${list.workid}_upload" class="shangchuan ml10" type="button" value="上 传" 
									onclick="return beforeSubmitForm('old_fileField','${list.workid}_frm','${list.workid}_old_uuid','${list.workid}_tp','${list.workid}');"
										 style="position:absolute;left:320px;border-bottom: 1px solid #000;border-right: 1px solid #000;border-top: 1px solid #D7D7D7;border-left: 1px solid #D7D7D7;color:#272222;background-color:#279F46;height:20px" />  
									<div style="position:absolute;left:420px;"><span id="${list.workid}_tp">0%</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									<span style="font-size:10px;">格式仅限doc、docx、txt、jpg、pdf、pps、ppsx、ppt和pptx</span></div>
									</form>
								</div>
							</td>
							<td width="10%" height="191" class="bg_eee" id="bg_eee_2_2" style="margin:0px auto;float:none">
							<input type="button" class="fr shanchu1" style="margin:0px auto;float:none" title="删除" id="${list.workid}_delete2" onclick="deleteselfpackage(this)"/> 
							<input type="button" class="fr tianjia" style="margin:0px auto;float:none" title="保存" id="${list.workid}_save"  onclick="updateold('${list.workid}')"/>
							</td>
						</tr>
						<%i++;%>
					</c:forEach>
		...
		//新增保存记录袋
		 function savepackage(div,day,valuenum,rid,zhuti,check){
			if(saveNow == 0){
				...
				var name = "学科学习过程记录";
						if(document.getElementById(valuenum).value.length<=200){
							if(document.getElementById(zhuti).value=="最多输入25字"){
								document.getElementById(zhuti).value = "";
							}
							if(document.getElementById(valuenum).value=="最多输入200字"){
								document.getElementById(valuenum).value = "";
							}
							saveNow = 1;
							Ext.Ajax.request({
								url:'${ctx}/learnprocess/LearnprocessWorksAction.a?doSave',
								method:'POST',
								...
								success:function(response,options){
									...
									if(response.responseText=="##"){
										...
										$('#new_subjectName').attr("id",saveid+"_subjectName");
										...
										if(check!="1"){
											...
											$("#"+saveid+"_content").css({float:"left",cursor:"text"});
								...
								params : {
									processdesc : document.getElementById(valuenum).value,
									subject : document.getElementById("new_subjectName").value,
									evaluateType1 : type1,
									evaluateType : type2,
									type : type,
			 ...
			 if (Sys.ie){
			 		if(Sys.ie<7.0){
						...
						return;
					     ...
					     var isrepeat = 0;
						var arr = new Array();
						/* var filetype = document.getElementById(id+"_textfield").value; */
								...
								if(pname.length<=25){
									if((ftype!=null&&ftype!="")&&(ftype=="doc"||ftype=="docx"||ftype=="txt"||ftype=="jpg"||ftype=="pdf"||ftype=="pps"||ftype=="ppsx"||ftype=="ppt"||ftype=="pptx")){
										...
										if(isrepeat!=1){
															...
															success: function(data){
																...
																if((json.state == "uploading"&&json.size<1024000)||json.state == "done"||json.state == "starting"){
													             	...
													             	if (json.state == 'done') {  
													                 	...
													                 	Ext.Ajax.request({
													    					url:'${ctx}/learnprocess/LearnprocessWorksAction.a?saveFile',
													    					method:'POST',
													    					defaults:{autoScroll: true},
													    					success:function(response,options){
													    						if(response.responseText=="##"){
													    						...
													    						for(var i=0;i<len[0].list2.length;++i)
													    						{
													    							if(len[0].list2[i].workid == id){
													    								for(var j=0;j<len[0].list2[i].attachListForFile.length;++j){
													    									...
													    									+"<a href='javascript:void(0);' style='color:black;' onclick='dodown(\""+len[0].list2[i].attachListForFile[j].attachid+"\",\""+len[0].list2[i].workid+"\")'>"+len[0].list2[i].attachListForFile[j].filename+"</a>"
													    									+"&nbsp;&nbsp;&nbsp;&nbsp;<a href='javascript:void(0);' title='删除' onclick='deleteselfattach(\""+len[0].list2[i].attachListForFile[j].attachid+"\",\""+len[0].list2[i].workid+"\")'><img src='${ctx}/images/upload_del.gif' style='height:10px; width:10px;' border='0'/></img></a>"
													    									+"</span></div>"
													    								/* 	+ len[0].list2[i].attachListForFile[j].filename
													    									...
													    									+"<input type='button' value='删 除' class='shangchuan fl ml10' onclick='deleteselfattach(\""+len[0].list2[i].attachListForFile[j].attachid+"\",\""+len[0].list2[i].workid+"\")'/></div>"; */
														    							}
													    							}
													    						}
											...
											alert("学科学习过程记录 栏目"+$("#"+id+"_num").html()+": 附件名重复!");	
											$("#"+id+"_upload").attr("disabled",false);
											$('#old_fileField').attr("id",beforeid);
										}
					...
					/*   alert("正在保存中..."); */
					  ...
					  if(idd=="new"){
					    	...
					    	if(tr.size()!=4){
							 ...
							 Ext.Ajax.request({
									url:'${ctx}/learnprocess/LearnprocessWorksAction.a?deleteLearnprocessWorks',
									method:'POST',
									defaults:{autoScroll: true},
									success:function(response,options){
												...
												var str="";
												 str+="<tr id='new_tr2'><td width='10%' height='40' id='new_num2' class='bg_eee'>1</td><td colspan='2' class='youjuzhong pr20 bg_eee'>"
													...
													+"</td></tr><tr><td colspan='2' id='new_td2' class='h50' style='width: 90%;'>"
												 	+"<input type='hidden' class='fl wenbenkuang670' id='new_topic2'  style='background: transparent; float:left;color:#BCBCBC;cursor:text;' />"
												 	+"<div class='downbox mt18'><span class='fl wenben85left'>科目：</span><div align='left'><select name='new_subjectName' id='new_subjectName'  style='background: transparent;border:1px solid #999;'>"
													...
													+"</select></div></div>"
												 	+"<div class='downbox mt18'><span class='fl wenben85left'>内容描述：</span><textarea id='new_content2' style='background: transparent; overflow:auto; height: 130px; width: 99%; border:1px solid #999; float:left;color:#BCBCBC;cursor:text;' onfocus='turnon(\""+"new_content2"+"\",this)' onblur='turnoff(\""+"new_content2"+"\",this)'>最多输入200字</textarea></div>"
			  ...
			  if (r==true){
				  var type1 = "1";
					var type2 = "1";
						type2 = document.getElementById("evaluateType2").value;
				  Ext.Ajax.request({
						url:'${ctx}/selfappraise/SelfAppAction.a?deleteSelfAttachLearn',
						method:'POST',
						defaults:{autoScroll: true},
						success:function(response,options){
							if(response.responseText=="1"){
		...
		function newdeleteselfattach(id,rid,rrid){
			  ...
			  if (r==true){
				  ...
				  Ext.Ajax.request({
						...
						success:function(response,options){
							if(response.responseText=="1"){
								alert_delete_failure("学科学习过程记录");
							}else{
								alert_delete_success("学科学习过程记录");
	    						...
	    						for(var i=0;i<len[0].list2.length;++i)
	    						{
	    							if(len[0].list2[i].workid == rid){
	    								for(var j=0;j<len[0].list2[i].attachListForFile.length;++j){
	    									...
	    									/* + len[0].list2[i].attachListForFile[j].filename
	    									+"<input type='button' value='下 载' class='shangchuan fl ml10'  onclick='dodown(\""+len[0].list2[i].attachListForFile[j].attachid+"\")'/>"
	    									+"<input type='button' value='删 除' class='shangchuan fl ml10' onclick='newdeleteselfattach(\""+len[0].list2[i].attachListForFile[j].attachid+"\",\""+len[0].list2[i].workid+"\",\""+rrid+"\")'/></div>"; */
		    							}
	    							}
	 		...
	 		if(document.getElementById("new_tr2")==undefined||document.getElementById("new_tr2")==null){
				...
				var tr_num=Math.floor(tr/2);
				 ...
				 str+="<tr id='new_tr2'><td width='10%' height='40' id='new_num2' class='bg_eee'>"+tr_num+"</td><td colspan='2' class='youjuzhong pr20 bg_eee'>"
					...
					+"</td></tr><tr><td colspan='2' id='new_td2' class='h50' style='width: 90%;'>"
				 	+"<input type='hidden' class='fl wenbenkuang670' id='new_topic2'  style='background: transparent; float:left;color:#BCBCBC;cursor:text;' />"
				 	+"<div class='downbox mt18'><span class='fl wenben85left'>科目：</span><div align='left'><select name='new_subjectName' id='new_subjectName'  style='border:1px solid #999;background: transparent;'>"
				 	+"<option value='语文'>语文&nbsp;&nbsp;&nbsp;&nbsp;</option>"
					+"<option value='英语'>英语&nbsp;&nbsp;&nbsp;&nbsp;</option>"
					+"<option value='数学'>数学&nbsp;&nbsp;&nbsp;&nbsp;</option>"
					+"<option value='思想政治'>思想政治&nbsp;&nbsp;&nbsp;&nbsp;</option>"
					...
					+"</select></div></div>"
				 	...
				 	+"<input id='new_upload' class='shangchuan ml10' type='button' value='上 传' onclick='checkupload()' style='position:absolute;left:320px;border-bottom: 1px solid #000;border-right: 1px solid #000;border-top: 1px solid #D7D7D7;border-left: 1px solid #D7D7D7;color:#272222;background-color:#279F46;height:20px' />"
				 	+"<div style='position:absolute;left:420px;'><span id='new_tp'>0%</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span style='font-size:10px;'>格式仅限doc、docx、txt、jpg、pdf、pps、ppsx、ppt和pptx</span></div>"
				 	+"</form></div></td><td width='"+width+"' height='191' class='bg_eee' id='bg_eee_2_2'>"
		...
		/* 	var id = window.event.srcElement.getAttribute('id'); */
			...
			if(idi!="new"){
				...
				var type2 = document.getElementById("evaluateType2").value;
						if(document.getElementById(idi+"_content").value.length<=200){
							if(document.getElementById(idi+"_content").value=="最多输入200字"){
								document.getElementById(idi+"_content").value = "";
							}
							var v_url='${ctx}/learnprocess/LearnprocessWorksAction.a?doUpdate'
							Ext.Ajax.request({
								url : v_url,
								method:'POST',
								defaults:{autoScroll: true},
								...
								params : {
									id : idi,
									processdesc : document.getElementById(idi+"_content").value,
									subject : document.getElementById(idi+"_subjectName").value,
									evaluateType1 : type1,
									evaluateType : type2,
									termId : document.getElementById("termId").value,
						...
						if(this_div.value.length<=200){
							...
							var v_url='${ctx}/learnprocess/LearnprocessWorksAction.a?doUpdate'
								  Ext.Ajax.request({
										...
										params : {
											id : id,
											processdesc : this_div.value,
											termId : document.getElementById("termId").value,
											subject : document.getElementById(id+"_subjectName").value,
		 ...
		 function addnew(trid,div){
			 ...
			 if(document.getElementById("new_tr1")==null){
				 var div = document.getElementById(div); 
					...
					+"</td><td width='"+width+"' class='bg_eee' id='bg_eee_2_1'><span class='fr btn'  style='margin:0px auto;float:none'><input type='button' class='fr shanchu1' style='margin:0px auto;float:none' title='删除' id='new_delete1' onclick='deleteselfapp(this)'/></span></td></tr>";
					$("#"+trid).before(str);

...
function exportExcel()
{
	...
	dStatus = uuid(); 
	window.location.href="${ctx}/export/RequirdCountExport.a?termId="+termId+ "&dStatus=" + dStatus+"&gradeyear="+gradeyear+"&schoolname="+schoolname+"&termName="+termName;
	ShowDiv(); 
...
<form action="${ctx}/needs/DiscodeNeedAppraisesAction.a?queryData" method="post">
...
<div id="tjb_main" style="overflow: visible;">
  ...
  <table width="100%" border="0" cellspacing="1" bgcolor="#999999" class="biaoge">
   ...
   <tr class="biaotou_zi">
     <td width="4%" rowspan="4" >学校</td>
     <td colspan="6" width="6%">新学期伊始的我</td>
     <td colspan="9" width="9%">学期结束的我</td>
     <td colspan="3" width="3%">思想道德</td>
     <td colspan="6" width="6%">学业成就</td>
     <td colspan="3" width="3%">合作与交流</td>
     <td colspan="3" width="3%">运动与健康</td>
     <td colspan="3" width="3%">审美与表现</td>
     <td colspan="3" width="3%">综合实践活动</td>
     <td colspan="6" width="3%">个性发展</td>
     </tr>
   <tr class="biaotou_zi">
     <td colspan="3" width="3%">自我评价</td>
     <td colspan="3" width="3%">我的发展目标</td>
     <td colspan="3" width="3%">自我评价</td>
     <td colspan="3" width="3%">班主任评语</td>
     <td colspan="3" width="3%">家长期望和寄语</td>
     <td colspan="3" width="3%">自我评价</td>
     <td colspan="3" width="3%">自我评价</td>
     <td colspan="3" width="3%">课程评语</td>
	 <td colspan="3" width="3%">他人评价</td>
     <td colspan="3" width="3%">体质健康</td>
     <td colspan="3" width="3%">审美与表现记录袋</td>
     <td colspan="3" width="3%">研究性学习基本情况</td>
     <td colspan="3" width="3%">自我评价</td>
     <td colspan="3" width="3%">特长与成果展示</td>
     </tr>
  <tr class="biaotou_zi">
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">班主任</td>
  	<td colspan="3" width="3%">家长</td>
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">教师</td>
  	<td colspan="3" width="3%">同学</td>
  	<td colspan="3" width="3%">云平台提取</td>
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">学生本人</td>
  	<td colspan="3" width="3%">学生本人</td>
	<td colspan="3" width="3%">学生本人</td>
  ...
  <tr class="biaotou_zi">
    ...
    <div style="height:295px;overflow-y:scroll;width:109%">
    <table width="100%" border="0" cellspacing="1" bgcolor="#999999" class="biaoge"> -->
    <c:forEach items="${datas}" var="titem" varStatus="count">
		<tr>
			  <td rowspan="1" width="4%">${titem.key}</td>
			  <td width="1%">${titem.value[START_ZWPJ]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[START_ZWPJ]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[START_ZWPJ]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[START_WDFZMB]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[START_WDFZMB]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[START_WDFZMB]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[END_ZWPJ]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[END_ZWPJ]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[END_ZWPJ]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[END_BZRPY]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[END_BZRPY]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[END_BZRPY]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[END_JZPYQW]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[END_JZPYQW]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[END_JZPYQW]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[SX_ZWPJ]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[SX_ZWPJ]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[SX_ZWPJ]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[XY_ZWPJ]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[XY_ZWPJ]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[XY_ZWPJ]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[XY_TRPJ_KECHENG_PINGYU]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[XY_TRPJ_KECHENG_PINGYU]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[XY_TRPJ_KECHENG_PINGYU]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[HZ_TRPJ_TX]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[HZ_TRPJ_TX]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[HZ_TRPJ_TX]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[TYPE_YDJKTZJK]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[TYPE_YDJKTZJK]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[TYPE_YDJKTZJK]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[SMYBX_JLD]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[SMYBX_JLD]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[SMYBX_JLD]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[ZHSJ_YJXX_JBQK]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[ZHSJ_YJXX_JBQK]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[ZHSJ_YJXX_JBQK]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[GXFZ_ZWPJ]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[GXFZ_ZWPJ]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[GXFZ_ZWPJ]["percentOverStudentNum"]}</td>
			  <td width="1%">${titem.value[GXFZ_CGZS]["totalStudentNum"]}</td>
			  <td width="1%">${titem.value[GXFZ_CGZS]["overStudentNum"]}</td>
			  <td width="1%">${titem.value[GXFZ_CGZS]["percentOverStudentNum"]}</td>
		   </tr>
</c:forEach>
<c:if test="${datas!= null&& fn:length(datas) gt 0 }">
  <tr>
    <td colspan="1" width="100">合计</td>

...
function checkOtherData(currentObj,num,topic){
	if($$.trim(currentObj.val())){
		return course_alert_failure_overLength(currentObj,num,topic);
	}
	return true;
}
...
function course_alert_failure_numCheck(currentTopic,num){
	alert_common('保存失败,'+currentTopic+':只能填写正数且长度不能超过'+num+'个','gritter-item');
}
function course_alert_failure_numCheck1(currentTopic,num){
	alert_common('保存失败,'+currentTopic+':只能填写正数且填写数值长度不能大于'+num+'','gritter-item');
}

function checkName(currentObj,currentTopic){
	if(/[#]/.test(currentObj.val())){		
		alert_common('保存失败,'+currentTopic+':不能为特殊字符#','gritter-item');
		return false;
	}else{
		return true;
	}
...
function checkNum(currentObj,currentTopic,num){
	if(/^-?\d+\.?\d{0,2}$/.test(currentObj.val())){
		if(currentObj.val().length>num){
			course_alert_failure_numCheck(currentTopic,num);
			return false;
		}else{
			if(currentTopic=="学时数"){
				var numData = parseFloat(currentObj.val());
				if(numData>1000){
					alert_common('保存失败,'+currentTopic+':填写数值不能大于'+1000+'','gritter-item');
					return false;
				}else{
					return true;
				}
...
function checkNum1(currentObj,currentTopic,num){
	if(/^-?\d+\.?\d{0,2}$/.test(currentObj.val())){
		if(currentObj.val().length>num){
			course_alert_failure_numCheck1(currentTopic,num);
			return false;
		}else{
			if(currentTopic=="学分"){
				var numData = parseFloat(currentObj.val());
				if(numData>6){
					alert_common('保存失败,'+currentTopic+':只能填写正数且填写数值不能大于'+6+'','gritter-item');
					return false;
				}else{
					return true;
				}
...
function checkInpu(currentObj,currentTopic){
	if(!$$.trim(currentObj.val())){
		...
		if("INPUT"==currentObj[0].tagName){
			if("模块名称"==currentTopic){
				//提示不能输入特殊字符#
				return checkName(currentObj,currentTopic);
			}else if("模块编号"==currentTopic){
				//提示“汉字、数字、字母长度均为20个”
				...
				//提示“汉字、数字、字母长度均为6个”
				return checkNum1(currentObj,currentTopic,4);
			}else{
				//提示“汉字、数字、字母长度均为60个”

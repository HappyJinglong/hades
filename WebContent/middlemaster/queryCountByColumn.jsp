...
function checkData(classId,termid,className){
	ShowDiv();
	window.location.href="${ctx }/middlemaster/QueryAppraiseAction.a?sectionCode=${sectionCode}&&classId="+classId+"&&termid="+termid+"&&className="+className;
}
function chooseTerm(){
	var zsTermId = $("#termId option:selected").val();
	var gradeNum = $("#termId option:selected").attr("title").substr(0, 1);
	ShowDiv();
	window.location.href="${ctx }/middlemaster/QueryCommentAction.a?sectionCode="+${sectionCode}+"&&zsTermId="+zsTermId+"&&isHistory=1&&gradeNum="+gradeNum;
}
		...
		<c:forEach items="${classinfo.value }"  var="classinfos" varStatus="vs">
			 <td>
			 	<c:choose>
			 		...
			 		<a href="#" onclick="checkData(${fn:split(classinfo.key,'@')[2]},${fn:split(classinfo.key,'@')[3]},'${classinfos}')"> <span class="red" >${classinfos }</span></a>
			 		</c:when>
			 		<c:otherwise>
			 			${classinfos }
			 		</c:otherwise>

...
function save_content(tt){
	...
	if(sectionName == "班主任评语"){
		if($.trim(content)=="" || $.trim(content)=="最多输入600字，班主任评语与学籍管理云平台的学生操行评语实时互通，只需在一个平台录入，两个平台皆可显示。"){
			isSave=0;
			tt.parent().css("color","");
			input_noticemaster(newId,600);
			return;
		}else{
...
function update_content(tt,num){
	...
	if(sectionName == "班主任评价"){
		if($.trim(val_now)=="" || $.trim(val_now)=="最多输入600字，班主任评语与学籍管理云平台的学生操行评语实时互通，只需在一个平台录入，两个平台皆可显示。"){
			isNull=1;
			return;
		}else{
...
function deleteContent(currentObj,stuDivObj){
	...
	Ext.Ajax.request({
		url:'${ctx}/middlemaster/MasterApriseDataAction.a?doDeleteMasterAppraisal',
		method:'POST',
		defaults:{autoScroll: true},
		success:function(response,options){
			var temp=response.responseText;
		    if(temp!="false"){
		    	...
		    	if(count==1){
			 		...
			 		if(sectionName == "班主任评语"){
			 			input_noticemaster(studentid+"_newAdd",600)
			 		}else{
		    	...
		    	if(count==1){
		    		...
		    		var sectionName = $("#sectionName").val();		    
		    		if(sectionName=="班主任评语"){
		    			if(textArea.val()=="最多输入600字，班主任评语与学籍管理云平台的学生操行评语实时互通，只需在一个平台录入，两个平台皆可显示。"){
...
function chooseClass(){
	...
	ShowDiv();
	window.location.href="${ctx}/middlemaster/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+evaluatetypeid+"&&sectionName="+sectionName+"&&classId="+classid+"&&subject="+subject;
}
//锚点选中数据
...
function chooseTerm(){
	...
	ShowDiv();
	window.location.href="${ctx}/middlemaster/MasterApriseDataAction.a?showAppriseList&&flag="+'${flag}'+"&&evaluatetypeid="+'${evaluatetypeid}'+"&&sectionName="+'${sectionName}'+"&&classId="+'${classId}'+"&&zsTermId="+zsTermId+"&&isHistory=1&&subject="+subject;
}
...
function chooseSub(){
	...
	var classid = $("#cid").val();
	ShowDiv();
	window.location.href="${ctx}/middlemaster/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+evaluatetypeid+"&&sectionName="+sectionName+"&&subject="+subject+"&&classId="+classid+"&&zsTermId="+zsTermId+"&&isHistory=1";
}
</script>
<body id="text_style">

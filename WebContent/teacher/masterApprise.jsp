...
function chooseTerm(){
	...
	ShowDiv();
	window.location.href="${ctx}/teacher/StudentApprisedAction.a?studentid="+'${studentid}'+"&key="+'${key}'+"&name="+'${name}'+"&classId="+'${classId}'+"&eduid="+'${eduid}'+"&isHistory=1"+"&zsTermId="+zsTermId;
}
var isNull = 0;
var isSave = 0;
var isDelete=0;
				 ...
				 if(temp!="false"){
					 ...
					 if(isDelete==1){
					   ...
					   if(deleteid==sectionCode){  
							deleteContent($("#"+temp+"_delete"),$("#"+sectionCode),'${ctx}/teacher/MasterApriseDataAction.a?doDeleteMasterAppraisal');
					   }else{
							alert_save_success($("#"+sectionCode+"_SN").html().replace("*",""), $("#"+temp+"_tpOne").children(":first").html());
							deleteContent($("#"+deleteid+"_delete"),$("#"+deleteid+"_delete").closest("table"),'${ctx}/teacher/MasterApriseDataAction.a?doDeleteMasterAppraisal');
					   }  
					 }else{
						 alert_save_success($("#"+sectionCode+"_SN").html().replace("*",""), $("#"+temp+"_tpOne").children(":first").html());
					 }
...
function deleteCountentPage(currentObj,tableObj){
	...
	if(flag){
		...
		deleteContent(currentObj,tableObj,'${ctx}/teacher/MasterApriseDataAction.a?doDeleteMasterAppraisal');
	}
}
...
function addNewContent_subject(currentObj){
	...
	var newContent ='<tr id="'+sectionCode+'_tpOne">'
    	...
    	+'</td>'
    	+'<td width="'+width+'" id="bg_eee_1_1" class="bg_eee"><span class="fr btn">'
       	+'<input id="'+sectionCode+'_delete" type="button" class="fr shanchu1" onclick="deleteCountentPage($(this),$(\'#'+sectionCode+'\'));"/>'
    	+'</span></td>'
  		+'</tr>';
  		currentObj.parent().parent().parent().before(newContent);
	...
	<c:forEach items="${sections}" var="section" varStatus="pVs">
	  ...
	  <c:if test="${section.sectionId ne '44'}">
	    <table id="${section.sectionId}" width="100%" border="0" cellspacing="1" bgcolor="#999999" class="biaoge">
		    <tr class="title_bg">
		    <td colspan="3" id="${section.sectionId}_SN">${section.sectionName}<c:if test="${section.sectionId eq '44'}"><span class="red">*</span></c:if></td>
	        </tr>

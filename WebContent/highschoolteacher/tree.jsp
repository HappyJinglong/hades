...
<script type="text/javascript">
var dataUrl = '${ctx}/highschoolteacher/TreeNodeAction.a?getTreeContent';	
var menutree;
...
function forward(){
	...
	if(commonFuncId=='null'||commonFuncId==''||commonFuncId=='4010'){
		id="student";
		$("#"+id).attr("href","${ctx }/highschoolteacher/StudentApprisedAction.a");
	}else{
...
var timeId = setInterval("clearTimeId()",5);
  function showStudentMenu(){
	    ...
	    /* $("#student").attr("href","${ctx }/highschoolteacher/StudentApprisedAction.a?showAppriseCount"); */
	    $("#student").attr("href","${ctx }/highschoolteacher/StudentApprisedAction.a");
  }
  function showTeacherMenu(){
	  self.parent.frames[2].ShowDiv && self.parent.frames[2].ShowDiv();
	    ...
	    $("#teacher").attr("href","${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=3020");
  }
...
<div id="pj_menu2"  style="display:none;width:100%;"> 
    <ul class="menu menu_zw" style="overflow:auto;width:100%;overflow-x:hidden; height: 100%;">
      <li class="cur"><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=3020" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId or commonFuncId eq '4010'}">id="student"</c:if> <c:if test="${not empty commonFuncId and commonFuncId ne '4010'}">id="student1003"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xiesx.png">思想道德</a></li> 
      <li><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=9999" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student9999"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xiesh.png">课程评语&nbsp;<span class="red">*</span></a></li>
      <li><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=8040" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1004"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xiecj.png">学业成就</a></li>  
      <li><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=4020" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1005"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xiehz.png">合作与交流</a></li>  
      <li><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=5020" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1006"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xieyd.png">运动与健康</a></li>  
      <li><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=6020" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1007"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xiesm.png">审美与表现</a></li>  
      <li><a href="${ctx }/highschoolteacher/StudentApprisedAction.a?showStudentCount&&sectionCode=7030" onclick="changeAStyle($(this))" <c:if test="${ empty commonFuncId}">id="student"</c:if> <c:if test="${not empty commonFuncId}">id="student1009"</c:if> target="main" class="nobdt"><img src="${ctx}/images/xiegx.png">个性发展</a></li> 

...
function chooseTerm(){
	...
	ShowDiv();
	window.location.href="${ctx}/highschoolteacher/StudentApprisedAction.a?studentid="+'${studentid}'+"&key="+'${key}'+"&name="+'${name}'+"&classId="+'${classId}'+"&eduid="+'${eduid}'+"&isHistory=1"+"&zsTermId="+zsTermId;
}
var isNull = 0;
var isSave = 0;
var isDelete=0;
...
function saveNewContent(currentObj,tableObj){
	...
	var subject = null;
	 ...
	 if(tempContent!="" && tempContent!="最多输入4000字")
	{
				 ...
				 if(temp!="false"){
					 ...
					 if(isDelete==1){
					   ...
					   if(deleteid==sectionCode){  
							deleteContent($("#"+temp+"_delete"),$("#"+sectionCode),'${ctx}/highschoolteacher/MasterApriseDataAction.a?doDeleteMasterAppraisal');
					   }else{
							alert_save_success($("#"+sectionCode+"_SN").html().replace("*",""), $("#"+temp+"_tpOne").children(":first").html());
							deleteContent($("#"+deleteid+"_delete"),$("#"+deleteid+"_delete").closest("table"),'${ctx}/highschoolteacher/MasterApriseDataAction.a?doDeleteMasterAppraisal');
					   }  
					 }else{
						 alert_save_success($("#"+sectionCode+"_SN").html().replace("*",""), $("#"+temp+"_tpOne").children(":first").html());
					 }
...
function updateContent(currentObj,tableObj,num){
		...
		if($.trim(newContent)=="" || $.trim(newContent)=="最多输入4000字"){
			isNull=1;
			return;
		}
...
function deleteCountentPage(currentObj,tableObj){
	...
	if(flag){
		...
		deleteContent(currentObj,tableObj,'${ctx}/highschoolteacher/MasterApriseDataAction.a?doDeleteMasterAppraisal');
	}
}
function changeBack(currentObj){
	...
	if(sectionCode=="9999"&&split_content=="content")
	{
...
function addNewContent(currentObj){
	...
	var newContent ='<tr id="'+sectionCode+'_tpOne">'
    	+'<td id="'+sectionCode+'_order" width="10%" height="40" class="bg_eee">'+(countTD+1)+'</td>'
   		+'<td colspan="2" class="youjuzhong pr20 bg_eee" >评价人：任课老师&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签名：${teacherName}&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
...
function addNewContent_subject(currentObj){
	...
	var newContent ='<tr id="'+sectionCode+'_tpOne">'
    	...
    	+'<option selected="selected" value="${list.subjectName }">${list.subjectName }&nbsp;&nbsp;&nbsp;&nbsp;</option> </c:if><c:if test="${status.index!=0}">'
    	+'<option value="${list.subjectName }">${list.subjectName }&nbsp;&nbsp;&nbsp;&nbsp;</option></c:if></c:forEach></select> </span></div>'
    	+'<div class="downbox mt18"><span class="fl wenben85left">课程评语：</span><textarea id="newAdd_'+sectionCode+'" idValue="" class="fl " style="border: 1px solid #999;background: transparent; height: 130px; width: 98.7%;margin-left:0px;"'
...
<div id="pj_ziwo_main">
	<c:forEach items="${sections}" var="section" varStatus="pVs">
	  ...
	  <c:if test="${section.sectionId ne '9999'}">
	    <table id="${section.sectionId}" width="100%" border="0" cellspacing="1" bgcolor="#999999" class="biaoge">
		    <tr class="title_bg">
		    <td colspan="3" id="${section.sectionId}_SN">${section.sectionName}<c:if test="${section.sectionId eq '9999'}"><span class="red">*</span></c:if></td>
	        </tr>
	        <c:if test="${fn:length(section.aInfos)!=0}">
		        <c:forEach items="${section.aInfos}" var="aInfo" varStatus="vs">
				    <tr id="${aInfo.apprasialid}_tpOne">
				      ...
				      <td colspan="2" class="youjuzhong pr20 bg_eee" >评价人：任课老师&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签名：<span id="${aInfo.apprasialid}_sign">${aInfo.appraser}</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		 ...
		 <c:if test="${section.sectionId eq '9999'}">
		      <table id="${section.sectionId}" width="100%" border="0" cellspacing="1" bgcolor="#999999" class="biaoge">
		    <tr class="title_bg">
				      ...
				      <td colspan="2" class="h50" style="background:#eee">
				        <div class="downbox mt18" >
                         ...
                         <c:forEach items="${subjectDtos }" var="list1" varStatus="status">
                           <c:if test="${aInfo.subject eq list1.subjectName}">
                               <option value="${list1.subjectName}" selected="selected">${list1.subjectName }&nbsp;&nbsp;&nbsp;&nbsp;</option>
                           </c:if>
                           <c:if test="${aInfo.subject!=list1.subjectName}">
                               <option value="${list1.subjectName }">${list1.subjectName }&nbsp;&nbsp;&nbsp;&nbsp;</option>
                           </c:if>
                         </c:forEach>
                         </select> </span>
                        </div>
	                    ...
	                    <textarea 
	                    style="border: 1px solid #999;background: transparent; height: 130px; width: 98.7%;margin-left:0px;"
		                id="${aInfo.apprasialid}_content"
		                idValue="${aInfo.apprasial }"
		                class="fl "
		                onblur="save($(this),$('#${section.sectionId}'));" onfocus="changeBack($(this));">${aInfo.apprasial }</textarea>
				        ...
				        <div class="downbox mt18">
                         ...
                         <c:forEach items="${subjectDtos }" var="list" varStatus="status">
                             <c:if test="${status.index==0}">
                              <option selected="selected" value="${list.subjectName }">${list.subjectName }&nbsp;&nbsp;&nbsp;&nbsp;</option>
                             </c:if>
                             <c:if test="${status.index!=0}">
                              <option value="${list.subjectName }">${list.subjectName }&nbsp;&nbsp;&nbsp;&nbsp;</option>
                             </c:if>
                        </c:forEach>
                        </select> </span>
                       </div>

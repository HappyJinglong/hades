...
function checkData(aObj,num){
	...
	var flag = vs%2;
	window.location.href="${ctx}/teacher/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+'${sectionCode}'+"&&sectionName="+'${sectionName}'+"&&classId="+cid+"&&zsTermId="+term+"&&isHistory=1";
}
function chooseTerm(){
	var zsTermId = $("#termId option:selected").val();
	var className = $("#termId option:selected").html();
	ShowDiv();
	window.location.href="${ctx }/teacher/StudentApprisedAction.a?showStudentCount&&sectionCode="+${sectionCode}+"&&zsTermId="+zsTermId+"&&isHistory=1&&className="+className;
}
</script>
<body>

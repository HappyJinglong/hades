...
function exportStuDetail()
{
	...
	dStatus = uuid();
	window.location.href = "${ctx}/export/SeniorStuApprasialCount.a?juniorDetailExport&gradeId="
				+ gradeId
				+ "&dStatus=" 
				+ dStatus
				+ "&classid="
...
<body>
<div class="dangqianwz" style="z-index:1001;position: fixed;padding-top: 10px;">
<form id="form" action="${ctx}/xspjtjmxb/StudentAppDetailAction.a?queryStatistData" method="post">
  ...
  <span >学期：
     <select name="termid" id='termid' class="  wenziliebiao100">
            ...
            <span ><input type="button" value="查 询" class="button ml10" onclick="getData();"/></span>&nbsp;&nbsp;&nbsp;
...
<div id="tjb_main">
  ...
  <table width="100%" border="0" cellspacing="1" bgcolor="#999999" class="biaoge">
   ...
   <tr class="biaotou_zi">
     ...
     <td colspan="2">新学期伊始的我</td>
     <td colspan="3">学期结束的我</td>
     <td colspan="5">思想道德</td>
     <td colspan="5">学业成就</td>
     <td colspan="5">合作与交流</td>
     <td colspan="5">运动与健康</td>
     <td colspan="5">审美与表现</td>
     <td colspan="5">综合实践活动</td>
     <td colspan="5">个性发展</td>
     </tr>
   <tr class="biaotou_zi">
     ...
     <td width="40" rowspan="2">家长期望和寄语</td>
     <td width="40" rowspan="2">自我评价</td>
...
<c:forEach items="${data}" var="obj">
  <tr>
   ...
   <td>${obj.zhsjhdYjxxxBaseInfoNum}</td>
   <td>${obj.zhsjhdYjxxxAchievementsNum}</td>
   <td>${obj.zhsjhdYjxxxMySelfAppraiseNum}</td>
   <td>${obj.zhsjhdSqfuYshsjBaseInfoNum}</td>
   <td>${obj.zhsjhdSqfuYshsjMySelfAppraiseNum}</td>

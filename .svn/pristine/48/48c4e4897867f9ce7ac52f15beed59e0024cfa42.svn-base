...
function checkData(aObj,num){
	...
	var cid = $("#"+pVs+"_cId").val();
	var flag = vs%2;
	window.location.href="${ctx}/master/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+'${sectionCode}'+"&&sectionName="+'${sectionName}'+"&&classId="+cid+"&&zsTermId="+'${zsTermId}'+"&&isHistory=1";
}
function chooseTerm(){
	...
	window.location.href="${ctx }/master/StudentApprisedAction.a?showStudentCount&&sectionCode="+${sectionCode}+"&&zsTermId="+zsTermId+"&&isHistory=1";
}
</script>
<body>
		...
		<c:forEach items="${classinfo.value }"  var="classinfos" varStatus="vs">
			 <td>
			 		...
			 		<c:if test="${vs.count==1 }">
			 			${fn:split(classinfos,'@')[0]}<input type="hidden"  id="${vsP.count}_cId" value="${fn:split(classinfos,'@')[1]}"/>
			 		</c:if>
			 </td>
		</c:forEach>  	
	</tr>

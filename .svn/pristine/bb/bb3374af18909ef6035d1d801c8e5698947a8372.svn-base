...
function save_content(tt){
	...
	if($.trim(content)=="" || $.trim(content)=="最多输入4000字"){
		isSave=0;
		tt.parent().css("color","");
		input_notice(newId,4000);
		return;
	}
	...
	Ext.Ajax.request({
		url:'${ctx}/highschoolteacher/MasterApriseDataAction.a?doInsertMasterAppraisal',
		method:'POST',
...
function update_content(tt,num){
	...
	if($.trim(val_now)=="" || $.trim(val_now)=="最多输入4000字"){
		isNull=1;
		return;
	}
	if(val_now.length>4000){
		isNull=1;
		apprasial_alert(4000);
		return;
	}
	Ext.Ajax.request({
		url:'${ctx}/highschoolteacher/MasterApriseDataAction.a?doUpadateMasterAppraisal',
		method:'POST',
		defaults:{autoScroll: true},
		success:function(response,options){
		 	var temp=response.responseText;
		    if(temp!="false"){
		    	if($("#"+appraisalid+"_sign")){
		    		$("#"+appraisalid+"_sign").html("${teacherName}");
		    	} 
...
function addContent(parentDiv,currentObj){
	...
	var tmp= '<table  class="xiechengsheng4" cellspacing="0" id="'+sid+'_tab">' 
		...
		+'</textarea>'
		+'<div class="fr btn" style="width:'+width+';" id="shanchu_btn" name="shanchu_btn"><input id="'+blank+'_delete" class="fr shanchu" type="button" style="margin-top:30px;margin-right:20px;" onclick="delete_content($(this),$(\'#'+sid+'_singleStu\'))"/></div></div>'
...
function deleteContent(currentObj,stuDivObj){
	...
	Ext.Ajax.request({
		url:'${ctx}/highschoolteacher/MasterApriseDataAction.a?doDeleteMasterAppraisal',
		method:'POST',
		defaults:{autoScroll: true},
		success:function(response,options){
			var temp=response.responseText;
		    if(temp!="false"){
		    	...
		    	if(count==1){
					...
					input_notice(studentid+"_newAdd",4000)
		    	...
		    	if(count==1){
		    		...
		    		if(textArea.val()=="最多输入4000字" || textArea.val()=="最多输入300字"){
...
function chooseClass(){
	...
	ShowDiv();
	window.location.href="${ctx}/highschoolteacher/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+evaluatetypeid+"&&sectionName="+sectionName+"&&classId="+classid+"&&subject="+subject;
}
//锚点选中数据
...
function chooseTerm(){
	...
	ShowDiv();
	window.location.href="${ctx}/highschoolteacher/MasterApriseDataAction.a?showAppriseList&&flag="+'${flag}'+"&&evaluatetypeid="+'${evaluatetypeid}'+"&&sectionName="+'${sectionName}'+"&&classId="+'${classId}'+"&&zsTermId="+zsTermId+"&&isHistory=1&&subject="+subject;
}
...
function chooseSub(){
	...
	var classid = $("#cid").val();
	ShowDiv();
	window.location.href="${ctx}/highschoolteacher/MasterApriseDataAction.a?showAppriseList&&flag="+flag+"&&evaluatetypeid="+evaluatetypeid+"&&sectionName="+sectionName+"&&subject="+subject+"&&classId="+classid+"&&zsTermId="+zsTermId+"&&isHistory=1";
}
</script>
<body id="text_style"> 
...
<div class="dangqianwz">
 	<span class="fl">当前位置：已完成评价学生->${sectionName}&nbsp;&nbsp;&nbsp;</span> 
    <span >班级：
     <select id="selectedId" name="select" class="  wenziliebiao100" onchange="chooseClass();">
            ...
            &nbsp;&nbsp;
            	<c:if test="${evaluatetypeid eq '9999'}">
                        ...
                        <span >课程：
				    		<select id="subSelected" name="select" class="  wenziliebiao100" onchange="chooseSub();">
					    			<c:forEach items="${subjectDtos }" var="subjct" varStatus="vs">
					    				<option    <c:if test="${subjct.subjectName eq  subject}">selected="selected"</c:if>   value="${subjct.subjectName}">${subjct.subjectName }</option>
					    			</c:forEach>
				            </select>  
            			</span> &nbsp;&nbsp;&nbsp;
            		</c:if>
...
<div id="pj_jiaoshi_main" name="${student.eduid}_main" style="<c:if test='${vst.index!=0 }'>display:none;</c:if>"> 
	<div id="${student.studentid}_singleStu">
    	...
    	<c:forEach items="${student.aInfos}" var="info" varStatus="vs">
    	    			   			<table  class="xiechengsheng4" cellspacing="0"  id="${info.apprasialid}_tab"> 
		           <tr> 
		                <td id="${info.apprasialid }_order" width="10%" height="40" class="xiechengsheng3">${vs.count}</td>
		                <td  class="xiechengsheng" >评价人：任课老师 &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;签名：<span id="${info.apprasialid}_sign">${info.appraser }</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
						                  日期：<input	id="${info.apprasialid}_Date" type="text" size="10" value="${empty info.signdate ? date_content : info.signdate }" idvalue="${info.signdate }" onchange="saveOrUpdateTime($(this));"
							onClick="WdatePicker()" /></td>
	                 </tr>
              </table>
		        <div class="name_con" id ="${info.apprasialid}_one" style="background:#eee">
	        		<input type="hidden" id="${info.apprasialid}_Sid" value="${info.studentid }"/>
		        	<textarea id="${info.apprasialid}_content"  style="background: transparent; border: 1px solid transparent; " idValue="${info.apprasial }"  class="fl neirong ml70 mt10"  onblur="saveOrUpdate($(this));" onfocus="changeBack($(this));">${info.apprasial }</textarea>
		       		<div class="fr btn" id="shanchu_btn" name="shanchu_btn">
		            	<input id="${info.apprasialid}_delete" type="button" class="fr shanchu" style="margin-top: 30px;margin-right: 20px;" onclick="delete_content($(this),$('#${student.studentid}_singleStu'))"/>
		            </div>
		        </div>
		          <div class="name_cxbox" id ="${info.apprasialid}_two" 
		          		<c:if test="${vs.count != fn:length(student.aInfos) or evaluatetypeid eq '22'}">
					                  	  style="display:none;"
				        </c:if>
		          >
			            <div class="name_cxright fr">
			                <div class="fr btn" id="three">
				                  <input id="${info.apprasialid}_add" type="button" class="fr zengjia mr15" onclick="addContent($('#${student.studentid}_div'),$(this))"/>
			                </div>
			            </div>
		        </div>
        </c:forEach>

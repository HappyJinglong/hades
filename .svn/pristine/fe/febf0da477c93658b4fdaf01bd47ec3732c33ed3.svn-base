...
<%@ include file="/common/div.jsp"%>
	<div class="dangqianwz">
		<span class="fl">当前位置：自我评价->综合实践活动->社区服务</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
		&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; <span>学期： <select
		...
		<div class="down mt18 pb30">
			...
			<div id="top">
				<table width="100%" border="0" cellspacing="1" bgcolor="#999999"
					...
					<tr class="title_bg">
						<td colspan="3">社区服务内容<span class="red">*</span></td>
					</tr>
					<tr id="new_tr">
					...
					<tr>
						<td colspan="2" id="new_td" class="h50" style="width: 90%;">
							<div class="downbox mt18">
	                    		<span class="fl wenben86left" style="width:145px">选择次数：</span>
	               					<select name="select" class=" fl  wenziliebiao100" id="new_xzcs" style="background: transparent;">
							...
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务社区名称：</span> <input type="text"
									class="fl wenbenkuang670" id="new_tm" style="background: transparent;float:left;color:#BCBCBC;cursor:text;" maxlength="120" onfocus="turnon120(event)" onblur="turnoff120(event)" value="最多输入120字"  />
							</div>
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务社区联系电话：</span> <input type="text"
							...
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务时数：</span> <input type="text"
									class="fl wenbenkuang670" id="new_zxss" style="background: transparent;float:left;color:#BCBCBC;cursor:text;" maxlength="25" onfocus="turnon25(event)" onblur="turnoff25(event)" value="最多输入25字" />
							</div>
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务日期：</span> <input type="text"
									class="fl wenbenkuang670" id="new_sslj" style="background: transparent;float:left;color:#BCBCBC;cursor:text;" maxlength="25" onfocus="turnon25(event)" onblur="turnoff25(event)" value="最多输入25字" />
							</div>
						...
						<td width="10%" height="191" id="bg_eee_4_1" class="bg_eee">
				...
				<table width="100%" border="0" cellspacing="1" bgcolor="#999999"
					...
					<tr>
						<td colspan="2" id="${list.practiceid}_td" class="h50" style="background:#eee;width: 90%;">
							<div class="downbox mt18">
		                   		<span class="fl wenben220left" style="width:145px">选择次数：</span>
		                		<select name="select" class=" fl  wenziliebiao100" id="${list.practiceid}_xzcs" style="background: transparent;"  onclick="changeBackgroundtd('${list.practiceid}')">
							...
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务社区名称：</span> <input type="text"
									class="fl wenbenkuang670" id="${list.practiceid}_tm"  style="background: transparent;" maxlength="120" onfocus="turnon120(event)" onblur="turnoff120(event)"
							...
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务社区联系电话：</span> <input type="text"
									class="fl wenbenkuang670" id="${list.practiceid}_hhr" style="background: transparent;" maxlength="120" onfocus="turnon120(event)" onblur="turnoff120(event)"
								 	value="${list.item8}" onclick="changeBackgroundtd('${list.practiceid}')"/>
							</div>
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务时数：</span> <input type="text"
									class="fl wenbenkuang670" id="${list.practiceid}_zxss" style="background: transparent;" maxlength="25" onfocus="turnon25(event)" onblur="turnoff25(event)"
								 	value="${list.item2}" onclick="changeBackgroundtd('${list.practiceid}')"/>
							</div>
							<div class="downbox mt18">
								<span class="fl wenben86left" style="width:145px">服务日期：</span> <input type="text"
									class="fl wenbenkuang670" id="${list.practiceid}_sslj" style="background: transparent;" maxlength="25" onfocus="turnon25(event)" onblur="turnoff25(event)"
							...
							<div class="downbox mt18"  id="${list.practiceid}_textareatd">
								...
								<c:if test="${empty list.practiceappraisalList}">
								 ...
								 <input type="button" title="添 加"  id="${list.practiceid}_addapp" style="margin-left:91.15%" class="xgg fl ml10" onclick="addtextarea(event)"/>
							</div>
...
function saveall(blur,check,event){//blur为1是点击保存按钮,0是失去焦点保存;check点击上传为1，不点击上传为0
	...
	if($("#new_tm").val()=="最多输入120字"){new_tm=""}
	if($("#new_hhr").val()=="最多输入120字"){new_hhr=""}
	if($("#new_zxss").val()=="最多输入25字"){new_zxss=""}
	if($("#new_sslj").val()=="最多输入25字"){new_sslj=""}
	if($("#new_content").val()=="最多输入4000字"){new_content=""}
		if(saveNow == 0){
			saveNow =1;
			if(blur=="1"&&check=="0"){
						if($("#new_content").val().length<=4000){
							if($("#new_tm").val()=="最多输入120字"){$("#new_tm").val("");}
							if($("#new_hhr").val()=="最多输入120字"){$("#new_hhr").val("");}
							if($("#new_zxss").val()=="最多输入25字"){$("#new_zxss").val("");}
							if($("#new_sslj").val()=="最多输入25字"){$("#new_sslj").val("");}
									...
									if(response.responseText=="##"){
										...
										if(blur=="1"){
											...
											if(check!="1"){
												...
												$("#"+saveid+"_xzcs").bind("focus",function(){
													changeBackgroundtd(saveid);
												});
												document.getElementById(saveid+"_td").style.backgroundColor ="#eee";
												alert_save_success("社区服务",$("#"+saveid+"_num").html());
											}
										}else{
											saveNow = 0;
										}
...
function updatePractices(id){
	if(is600app==0){
		if($("#"+id+"_content").val().length<=4000){
			if($("#"+id+"_tm").val()=="最多输入120字"){$("#"+id+"_tm").val("");}
			if($("#"+id+"_hhr").val()=="最多输入120字"){$("#"+id+"_hhr").val("");}
			if($("#"+id+"_zxss").val()=="最多输入25字"){$("#"+id+"_zxss").val("");}
			if($("#"+id+"_sslj").val()=="最多输入25字"){$("#"+id+"_sslj").val("");}
			...
			Ext.Ajax.request({
				...
				success:function(response,options){
					...
					if(temp=="##"){
						...
						alert_save_success("社区服务",$("#"+id+"_num").html());
					}
				},
				params : {
					id : id,
									...
									if(tr.size()==4){
										...
										$("#"+idd+"_tr").remove();
										 ...
										 str+="<tr id='new_tr'><td width='10%' height='40' id='new_num' class='bg_eee'>"+"1"+"</td><td colspan='2' class='youjuzhong pr20 bg_eee'>"
										 	...
										 	+"<div class='downbox mt18'><span class='fl wenben220left' style='width:145px'>选择次数：</span>"
				                    		+"<select name='select' class=' fl  wenziliebiao100' id='new_xzcs' style='background: transparent;'>"
				               				+"<option value='第一次'>第一次</option><option value='第二次'>第二次</option><option value='第三次'>第三次</option>"	
					          				+"</select></div>"		
										 	+"<div class='downbox mt18'><span class='fl wenben86left' style='width:145px'>服务社区名称：</span> <input type='text' class='fl wenbenkuang670' id='new_tm' maxlength='120' style='background: transparent;float:left;color:#BCBCBC;cursor:text;' onfocus='turnon120(event)' onblur='turnon120(event)' value='最多输入120字'/></div>"
										 	+"<div class='downbox mt18'><span class='fl wenben86left' style='width:145px'>服务社区联系电话：</span> <input type='text' class='fl wenbenkuang670' id='new_hhr' maxlength='120'style='background: transparent;float:left;color:#BCBCBC;cursor:text;' onfocus='turnon120(event)' onblur='turnoff120(event)' value='最多输入120字'/></div>"
										 	+"<div class='downbox mt18'><span class='fl wenben86left' style='width:145px'>服务时数：</span> <input type='text' class='fl wenbenkuang670' id='new_zxss' maxlength='25' style='background: transparent;float:left;color:#BCBCBC;cursor:text;' onfocus='turnon25(event)' onblur='turnoff25(event)' value='最多输入25字'/></div>"
										 	+"<div class='downbox mt18'><span class='fl wenben86left' style='width:145px'>服务日期：</span> <input type='text' class='fl wenbenkuang670' id='new_sslj' maxlength='25' style='background: transparent;float:left;color:#BCBCBC;cursor:text;' onfocus='turnon25(event)' onblur='turnoff25(event)' value='最多输入25字'/></div>"
										 	+"<div class='downbox mt18'><span class='fl wenben86left'>内容摘要：</span><textarea id='new_content' class='wenbenyu112' style='background: transparent; float:left;color:#BCBCBC;cursor:text;' onfocus='turnon4000(event)' onblur='turnoff4000(event)'>最多输入4000字</textarea></div>"
										 	...
										 	+"</td><td height='191' id='bg_eee_4_1' class='bg_eee'>"
										 	+"<input type='button' class='fr shanchu1' title='删除' id='new_delete' onclick='deleteselfall(event)' style='margin:0px auto;float:none'/>"
										 	+"<input type='button' class='fr tianjia' title='保存'  style='margin:0px auto;float:none' id='new_save' onclick='check(event)'/>"
										 	+"</td></tr>"
		...
		var tr_num=Math.floor(tr/2);
		 ...
		 str+="<tr id='new_tr'><td width='10%' height='40' id='new_num' class='bg_eee'>"+tr_num+"</td><td colspan='2' class='youjuzhong pr20 bg_eee'>"
			...
			+"</select></div>"
		 	...
		 	+"<input name='new_fileField' id='new_fileField' type='file' style='position:absolute;background: transparent;left:80px;' />"
		 	+"<input id='new_upload' class='shangchuan ml10' type='button' value='上 传' onclick='checkupload()' style='position:absolute;left:320px;border-bottom: 1px solid #000;border-right: 1px solid #000;border-top: 1px solid #D7D7D7;border-left: 1px solid #D7D7D7;color:#272222;background-color:#279F46;height:20px' /> "
		 	+"<div style='position:absolute;left:420px;'><span id='new_tp'>0%</span>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp <span style='font-size:10px;'>格式仅限doc、docx、txt、jpg、pdf、pps、ppsx、ppt和pptx</span></div>"
		 	+"</form></div><div class='downbox mt18' id='new_textareatd'>"
		 	+"<span class='fl wenben86left'>自我评价：</span><textarea class='wenbenyu111' id='new_contentapp' onblur='checksaveapp(event)' style='background: transparent;float:left;color:#BCBCBC;cursor:text;'  onfocus='turnon600(event)'>最多输入600字</textarea>"
		 	+"<input type='button' title='删 除'  id='new_deleteapp' class='del fl ml10' onclick='deletepracticeappraisal(event)' style='margin-top:45px;margin-left:1px;'/>"
		 	+"<input type='button' title='添 加' style='margin-left:91.15%' id='new_addapp'  class='xgg fl ml10'  onclick='addtextarea(event)'/></div>"
		 	+"</td><td height='191' width='"+tdWidth+"' class='bg_eee' id='bg_eee_4_1'>"
		 	+"<input type='button' class='fr shanchu1' title='删除' id='new_delete' onclick='deleteselfall(event)' style='margin:0px auto;float:none'/>"
		 	+"<input type='button' class='fr tianjia' title='保存'  style='margin:0px auto;float:none' id='new_save' onclick='check(event)'/>"
		 	+"</td></tr>"
		 $("#addlocation").before(str);

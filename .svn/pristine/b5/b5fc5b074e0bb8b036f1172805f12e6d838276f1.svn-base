...
function initStudentInfos(){
	...
	var str = "";				
	str += "<ul class='name'>";	
	<c:forEach items="${partInfoDtoListClassStuder}" var="studentInfo" varStatus="vs">
	       var s=${vs.index};
        if(0==s){
	    	 var studentid3 = "${studentInfo.studentid}";
	      }
	     var name   = "${studentInfo.pname}";
		var eduId  = "${studentInfo.eduid}";
		var status = "${studentInfo.contnumber}";
		var studentid = "${studentInfo.studentid}";
		var count = "${vs.index}";
		if(count==0){
			str += "<li style='background:gray;' onclick='findById(this,\""+eduId+"\",\""+name+"\","+studentid+")'><span title='"+name+"' style='cursor:pointer'>";
		}else{
			str += "<li onclick='findById(this,\""+eduId+"\",\""+name+"\","+studentid+")'><span title='"+name+"' style='cursor:pointer'>";
		}
		if(0<status){//判断状态
	...
	str += "</ul>";									
	divdsl.append(str); 
		 $("#"+studentid3+"_weizhi").css("display","block");
   	     $("#"+studentid3+"_div_weizhi").css("display","block");
	
}  
	  
function findById(obj,eduId,name,studentid){
  var ed=$("#eduId_danqian").val();
  if(ed==eduId){
	  return;
  }else{
		$("li").css("background","");
		$(obj).css("background","gray");
		var textareas=$("body").find("div");	
		for(var i=0;i<textareas.size();++i)
		{ 
			var id_content=$(textareas[i]).attr("id");
			 if(id_content.indexOf("weizhi")>=0) 
			 {  
		...
		//alert(studentid);
		$("#eduId_danqian").val(eduId);
		$("#evaluateName_danqian").val(name);
		$("#"+studentid+"_weizhi").css("display","block");
		$("#"+studentid+"_weizhi_name_pic").css("display","block");
		$("#"+studentid+"_weizhi_name").css("display","block");
		$("#"+studentid+"_div_weizhi").css("display","block");
		
	/* 	<div class="s_content" id="${stud.studentid}_div_weizhi"  style="background-color:rgb(39, 159, 70); margin-top: 19px;<c:if test='${statused.index!=0 }'>display:none;</c:if>" >
		<div  style="position:absolute;z-index:100;" id="${stud.studentid}_weizhi_name_pic"  class="name_pic"><img class="img-padding" src="${stud.photoUrl}"/></div>
		 <div class="title_xie" id="${stud.studentid}_weizhi_name" >${stud.name} </div>
	    </div> */
  }
} 
...
var dectestatic="notdelct";
	...
	function addApprasial(studentid,num){ 
	 ...
	 function save_process(){
		...
		var name1=$("#evaluateName_danqian").val();
		if(trimLeng>0){
		if(ss<=600){
	    	...
	    	Ext.Ajax.request({
				url:'CzPlayAndHealthXinAction.a?doUpdataOtherProcessNull',
				method:'POST',
					    ...
					    {   
					    	...
					    	var evaluatedPersonID=document.getElementById("new_studentid").value;
					    	  if(dectestatic=="delct"){
						       			...
						       			Ext.lib.Ajax.defaultPostHeader = 'application/x-www-form-urlencoded;charset=UTF-8';
							    		Ext.Ajax.request({
										url:'${ctx}/student/CzPlayAndHealthXinAction.a?deleApprasial',
										method:'POST',
										defaults:{autoScroll: true},
										success:function(response,options){
										...
										params : {
											part_id:pid,
											  two_part_id : $("#two_part_id_danqian").val(),
											  eduId:$("#eduId_danqian").val(),
											  evaluateName:$("#evaluateName_danqian").val(),
											  termId:$("#termId_danqian").val()
										    	...
										    	if(part_info.indexOf(">")!= -1)  
										    	 {  
										    		part_info=part_info.replaceAll(">","&gt")
										    	 } 
										    	if(part_info.indexOf("<")!= -1)  
										    	 {  
										    		part_info=part_info.replaceAll("<","&lt")
										    	 } 
										        ...
										        +"<div class='name_con ' style='background:#eee;' id='"+evaluatedPersonID+"_ "+newnum+"_boder'>"
											        	    +"<div class='fl neirong ml70 mt10' id='wuyong1_"+newnum+"' >"                         
											        	         +"<div  id='"+evaluatedPersonID+"_"+newnum+"_content' style='height:120px; margin-top:0px;  width:100%;pxoverflow:auto;' idvalue=''  >" 
											          ...
											          xianshi.innerHTML=newSuccessdiv; 
											            var part_infos=$("#new_apprise_content").val();
														 $("#"+evaluatedPersonID+"_"+newnum+"_content").attr("idvalue",part_infos); 
														 ...
														 $('#new_apprise').css("display","none"); 
															...
															// document.getElementById("apperedxiel_"+studentid+"_"+id).setAttribute("idvalue",document.getElementById("apperedxiel_"+studentid+"_"+id).value);
															 readying();
		...
		if(id==null||id==""){
			...
			if(flag==true){ 
				...
				if(apple!=content_div_full){
					Ext.lib.Ajax.defaultPostHeader = 'application/x-www-form-urlencoded;charset=UTF-8';
					Ext.Ajax.request({
						url:'CzPlayAndHealthXinAction.a?doUpdataOtherProcess',
						method:'POST',
						success:function(response,options){
							  var len=Ext.util.JSON.decode(response.responseText);
							    var success=len.success.replace( /^\s+|\s+$/g,"");
									  ...
									  var len=Ext.util.JSON.decode(response.responseText);
									    ...
									    {    
									    	 if(dectestatic!="delct"){
									    		 ...
									    		 var te=$("#eduId_danqian").val()
									    		 $("#"+te+"_libiao").css("color","");
									    		 $("#"+te+"_libiao").css("color","green");
	...
	function failure(document_object)
{
 ...
 function changeterm(){
		...
		var evaluateType= $("#two_part_id_danqian").val();
		url="${ctx}/student/CzPlayAndHealthXinAction.a?two_part_id="+evaluateType+"&termId="+term;
	...
	if(id==null||id==""||id==0)
		{
			...
			{   
				...
				Ext.Ajax.request({
					url:'${ctx}/student/CzPlayAndHealthXinAction.a?doUpadateTime',
					method:'POST',
					defaults:{autoScroll: true},
					success:function(response,options){
					},
...
<body id="text_style">
 ...
 <div class="s_content">
		<div class="nj">
			${className}
			<input  type="hidden" id="eduId_danqian" value="${eduId}"/>
			<input  type="hidden" id="evaluateName_danqian" value="${evaluateName}"/>
			<input  type="hidden" id="termId_danqian" value="${termId}"/>
			<input  type="hidden" id="two_part_id_danqian" value="${two_part_id}"/>
		</div> 
		<div class="ns">
 ...
 <div id="xian" ></div>
	        	...
	        	<div class="name_con">
		        	 <div class="fl neirong ml70 mt10" > 
		        	       <div style="height:120px; margin-top:0px;  width:100%;pxoverflow:auto;">  
		        			 <textarea   id="new_apprise_content"  style="height:120px; width:100%;overflow:auto;" onclick="romvezi()" onblur="return saveApprasial()"></textarea> 
						   </div> 
						 </div> 
  ...
  <!-- <div id="pj_jiaoshi_main"> -->
        <c:forEach items="${listApraisal}" var="stud"  varStatus="statused">
				<%-- <div id="${stud.studentid}_weizhi_name_pic" style="<c:if test='${statused.index!=0 }'>display:none;</c:if>" class="name_pic"><img class="img-padding" src="${ctx}/imgs/pic1.png"/></div> --%>
		    	<div class="s_content" id="${stud.studentid}_div_weizhi"  style="background-color:rgb(39, 159, 70); margin-top: 19px;display:none;">
		    		<div  style="position:absolute;z-index:100;" id="${stud.studentid}_weizhi_name_pic"  class="name_pic"><img class="img-padding" src="${stud.photoUrl}"/></div>
		    		 <div class="title_xie" id="${stud.studentid}_weizhi_name" >${stud.name} </div>
		    	</div>
		       ...
		       <%-- <div class="s_content" style="background-color:rgb(39, 159, 70);">
				 	<div class="name_pic" > <img class="img-padding" id="photoUrl" src="${photoUrl}"/> </div> 
				  	<div class="title_xie" style="overflow:hidden; width:90.1%;"  id="evaluateName_name">${evaluateName} </div>
 				</div> --%>
		       
				 
				 <div class="layout"  id="${stud.studentid}_weizhi" style=" display:none;">
		        	<div id="pj_jiaoshi_main"  >
		        	<div id="${stud.studentid}_xie">
		        	<c:forEach items="${stud.czaInfos}" var="list"  varStatus="status">
		        	   <div  id="${list.part_id}_dele_${status.index+1}">
		        	   
		        	  <table style="z-index:-1;" class="xiechengsheng4" cellspacing="0"> 
			           <tr> 
		        ...
		        <div class="name_con" style="background:#eee;" id="${list.studentid}_${status.index+1}_boder">
		        	    <div class="fl neirong ml70 mt10" id="wuyong1_${status.index+1}">
		        	         <div  style="height:130px; width:100%; margin-top:0px; pxoverflow:auto;"  id="${list.studentid}_${status.index+1}_content" idvalue="${list.part_info}"> 
		                       <textarea id="apperedxiel_${list.studentid}_${status.index+1}" style="background: transparent; border-style: none; height: 120px; width:100%;" onclick="return change_content_is_available(${list.part_id},${list.studentid},${status.index+1},true);">${list.part_info}</textarea>
					        </div> 
				       </div>
		           ...
		           <div class="name_con " id="wuyong4_${status.index+1}">
		        	    <div class="fl neirong ml70 mt10"  id="wuyong5_${status.index+1}">
						  <div  style="height:130px; margin-top:0px;  width:100%;pxoverflow:auto;"  id="${list.studentid}_${status.index+1}_content" idvalue="${list.part_info}"> 
								<textarea id="new_apprise_content_xiel_${list.studentid}_${status.index+1}" style="background: transparent; border-style: none; height: 120px; width: 100%;" onclick="return change_content_is_availableNull(${list.part_id},${list.studentid},${status.index+1},true)">${list.part_info}</textarea>
					      </div> 
						</div>
